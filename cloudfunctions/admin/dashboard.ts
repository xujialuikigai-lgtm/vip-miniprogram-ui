...
/** 待办订单状态集合（需要管理员关注/处理） */
const TODO_STATUSES: OrderStatus[] = [
  OrderStatus.ACTIVATING,
  OrderStatus.API_FAILED,
  OrderStatus.REFUNDING,
];
...
export async function handleDashboard(
  db: any,
  event: any
): Promise<CloudFunctionResult<DashboardResult>> {
  const now = new Date();
 ...
 * 计算今日核心指标（10.1）
 */
function computeStats(orders: Order[], todayStart: Date, now: Date): DashboardStats {
  const todayPaid = orders.filter(
    (o) => isPaidOrder(o) && inRange(toTime(o.createdAt), todayStart.getTime(), now.getTime())
  );

  const todaySales = round2(todayPaid.reduce((sum, o) => sum + (o.amount || 0), 0));
  const todayOrderCount = todayPaid.length;
  const successCount = todayPaid.filter((o) => o.status === OrderStatus.SUCCESS).length;
  // 开通成功率 = 开通成功订单数 ÷ 已支付总订单数 × 100%
  const successRate = todayOrderCount > 0 ? round1((successCount / todayOrderCount) * 100) : 0;
  const apiFailedCount = todayPaid.filter((o) => o.status === OrderStatus.API_FAILED).length;
 ...
 */
function computeDailyTrend(orders: Order[], todayStart: Date): DailyTrendItem[] {
  ...
  for (let i = 6; i >= 0; i--) {
    ...
    const dayOrders = orders.filter(
      (o) => isPaidOrder(o) && inRange(toTime(o.createdAt), dayStart.getTime(), dayEnd.getTime(), false)
    );
 ...
 */
function computeCategoryRatio(orders: Order[]): CategoryRatioItem[] {
 ...
 */
function computeRanking(orders: Order[], sevenDayStart: Date): RankingItem[] {
  const sevenDayOrders = orders.filter(
    (o) => isPaidOrder(o) && toTime(o.createdAt) >= sevenDayStart.getTime()
  );
 ...
 */
async function fetchOrdersSince(db: any, since: Date): Promise<Order[]> {
  const _ = db.command;
  const pageSize = 100;
  ...
  while (skip < maxRecords) {
    ...
    const batch = (res && Array.isArray(res.data) ? res.data : []) as Order[];
    all.push(...batch);
 ...
 * - 最多50条，按创建时间倒序
 */
async function fetchTodoOrders(db: any, todoStatus?: string): Promise<TodoOrderItem[]> {
  const _ = db.command;
  const statusCond =
    todoStatus && TODO_STATUSES.includes(todoStatus as OrderStatus)
      ? todoStatus
      : _.in(TODO_STATUSES);
  ...
  const list = (res && Array.isArray(res.data) ? res.data : []) as Order[];
  return list.map((o) => ({
    orderId: o.orderId,
    productName: o.productName,
    status: o.status,
 ...
 * 查询最近20条审计日志（10.5）
 */
async function fetchRecentAuditLogs(db: any): Promise<AuditLogItem[]> {
  const res = await db
  ...
  const list = (res && Array.isArray(res.data) ? res.data : []) as AuditLog[];
  return list.map((log) => ({
 ...
 */
function inRange(t: number, start: number, end: number, inclusiveEnd = true): boolean {
...
function formatBeijingMMDDHHmm(value: Date | string): string {
