...
import { CloudFunctionResult, PaginationResult } from './shared/types/api';
...
export interface AuditLogQuery {
  ...
  /** 按关联订单号筛选 */
  orderId?: string;
  /** 起始时间（含），ISO 8601 字符串或可被 Date 解析的值 */
  startTime?: string;
  /** 结束时间（含），ISO 8601 字符串或可被 Date 解析的值 */
  endTime?: string;
}
...
export async function handleAuditLogs(
  db: any,
  event: AuditLogQuery
): Promise<CloudFunctionResult<PaginationResult<AuditLog>>> {
  const _ = db.command;
  ...
  const total = (countRes && typeof countRes.total === 'number' ? countRes.total : 0) as number;
 ...
 */
function buildWhere(_: any, query: AuditLogQuery): Record<string, any> {
...
/** 判断是否为合法的审计操作类型枚举值 */
function isValidAuditType(type: string): boolean {
