...
exports.refreshOrder = refreshOrder;
// 订单刷新进度逻辑
...
function mapShunshiStatus(shunshiStatus) {
    switch (shunshiStatus) {
        case constants_1.ShunshiOrderStatus.SUCCESS: // 3
            return { status: order_1.OrderStatus.SUCCESS, desc: '开通成功', terminal: true };
        case constants_1.ShunshiOrderStatus.CANCELLED: // 4
            return { status: order_1.OrderStatus.API_FAILED, desc: '开通失败（订单已取消）', terminal: true };
        case constants_1.ShunshiOrderStatus.REFUNDED: // 5
            return { status: order_1.OrderStatus.API_FAILED, desc: '开通失败（已退款）', terminal: true };
        default: // 1 待处理 / 2 处理中
            return { status: order_1.OrderStatus.ACTIVATING, desc: '开通处理中', terminal: false };
    }
}
/**
 ...
 * @param order - 原始订单
 */
function maskOrderAccount(order) {
    const maskedAttach = {};
    const attach = order.attach || {};
    for (const key of Object.keys(attach)) {
        const value = attach[key];
        maskedAttach[key] = typeof value === 'string' ? (0, mask_1.maskAccount)(value) : value;
    }
    return Object.assign(Object.assign({}, order), { attach: maskedAttach });
}
/**
 ...
 * @param params - 刷新入参
 */
async function refreshOrder(db, openid, params) {
    ...
    try {
        ...
        info = await withTimeout(client.queryOrder({ ordersn: order.shunshiOrderSn }), REFRESH_TIMEOUT_MS);
    }
    catch (err) {
        // 记录失败审计日志（不阻断返回）
