...
import { WechatPayClient, PayParams, WechatPayError } from './wechatpay';
import { isMockMode } from './shared/constants';
...
export async function unifiedOrder(
...
): Promise<CloudFunctionResult<UnifiedOrderData>> {
  ...
  if (isMockMode()) {
    ...
    const mockTimeline: TimelineNode[] = [
      { status: OrderStatus.PAID, time: now, desc: '[Mock] 模拟支付成功' },
      { status: OrderStatus.ACTIVATING, time: now, desc: '[Mock] 模拟提交开通' },
      { status: OrderStatus.SUCCESS, time: now, desc: '[Mock] 模拟开通成功' }
    ];
    ...
    return { success: true, data: { payParams: mockPayParams, mock: true } as any };
  }

  const client = payClient || new WechatPayClient();
  let payParams: PayParams;
  try {
    ...
    const errCode = err instanceof WechatPayError ? `WXPAY_${err.code}` : 'WXPAY_FAILED';
    return {
      success: false,
      errCode,
      errMsg: '支付发起失败，请重试'
