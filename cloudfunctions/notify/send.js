...
const TEMPLATE_ENV_KEYS = {
    [types_1.NotifyType.ACTIVATION_SUCCESS]: 'WX_SUBSCRIBE_TPL_ACTIVATION_SUCCESS',
    [types_1.NotifyType.ACTIVATION_FAILED]: 'WX_SUBSCRIBE_TPL_ACTIVATION_FAILED',
    [types_1.NotifyType.REFUND_SUCCESS]: 'WX_SUBSCRIBE_TPL_REFUND_SUCCESS',
};
/**
 ...
 */
const UNSUBSCRIBED_ERR_CODES = new Set([43101]);
/**
 * 从环境变量解析模板ID。
 ...
 * @param params    发送参数
 */
async function send(cloudApi, params) {
    var _a;
    ...
    if (!templateId) {
        ...
        const log = (0, logger_1.createAuditLog)({
            ...
            errorMsg: `未找到通知类型 ${(_a = params.type) !== null && _a !== void 0 ? _a : '-'} 对应的模板ID环境变量`,
        });
    ...
    try {
        ...
        const errCode = res && typeof res.errCode === 'number' ? res.errCode : 0;
        if (errCode === 0) {
            // 发送成功
            const log = (0, logger_1.createAuditLog)({
                type: audit_1.AuditType.STATUS_UPDATE,
