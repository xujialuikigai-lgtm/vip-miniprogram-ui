...
import { verifyCallbackSign } from './shared/utils/sign';
...
import { ShunshiOrderStatus, BROADCAST_MAX_COUNT } from './shared/constants';
 ...
 * @returns 回调参数对象
 */
function parseCallbackBody(event: any): Record<string, any> {
  ...
  if (typeof event.body === 'string') {
    ...
    return querystring.parse(raw) as Record<string, any>;
  }

  // body 为对象：直接使用
  if (event.body && typeof event.body === 'object') {
    return event.body as Record<string, any>;
  }
 ...
 */
function mapShunshiStatus(shunshiStatus: number): MappedStatus | null {
  switch (shunshiStatus) {
    case ShunshiOrderStatus.SUCCESS: // 3 交易成功
      return { status: OrderStatus.SUCCESS, desc: '开通成功' };
    case ShunshiOrderStatus.CANCELLED: // 4 交易取消
      return { status: OrderStatus.API_FAILED, desc: '开通失败（订单已取消）' };
    case ShunshiOrderStatus.REFUNDED: // 5 退款
 ...
 */
function extractMaskedAccount(order: Order): string {
 ...
 */
async function writeBroadcastCache(db: any, order: Order): Promise<void> {
  try {
 ...
 */
async function triggerNotify(order: Order, success: boolean): Promise<void> {
  try {
 ...
 */
export async function main(event: any, context: any): Promise<string> {
  ...
  if (!apikey || !verifyCallbackSign(params, apikey)) {
  ...
  if (!order) {
    ...
    const notFoundLog = createAuditLog({
      type: AuditType.SHUNSHI_CALLBACK,
      operator: 'system',
      orderId: externalOrderno || ordersn || undefined,
      action: '顺势回调未匹配到订单',
    ...
    await writeAuditLog(db, notFoundLog);
    return 'ok';
  }
  ...
  if (!mapped) {
    ...
    const processingLog = createAuditLog({
      type: AuditType.SHUNSHI_CALLBACK,
      operator: 'system',
      orderId: order.orderId,
    ...
    await writeAuditLog(db, processingLog);
    return 'ok';
  }
  ...
  if (order.status === mapped.status || finalStatuses.indexOf(order.status) !== -1) {
    const idempotentLog = createAuditLog({
      type: AuditType.SHUNSHI_CALLBACK,
      operator: 'system',
      orderId: order.orderId,
    ...
    await writeAuditLog(db, idempotentLog);
    return 'ok';
  }

  // 6. 组装更新数据
  const now = new Date();
  const node: TimelineNode = { status: mapped.status, time: now, desc: mapped.desc };
  ...
  try {
    ...
    const updateFailLog = createAuditLog({
      type: AuditType.SHUNSHI_CALLBACK,
      operator: 'system',
      orderId: order.orderId,
      action: '顺势回调更新订单失败',
    ...
    await writeAuditLog(db, updateFailLog);
    // 更新失败返回 fail，等待顺势重试
    return 'fail';
  }
  ...
  const successLog = createAuditLog({
    ...
    operator: 'system',
    orderId: order.orderId,
    action: mapped.status === OrderStatus.SUCCESS ? '顺势回调：开通成功' : '顺势回调：开通失败',
    detail: { shunshiStatus, targetStatus: mapped.status },
    result: 'success'
  });
  await writeAuditLog(db, successLog);

  // 8. 副作用：成功写播报 + 通知，失败仅通知（合并最新订单字段后处理）
  const latestOrder: Order = { ...order, ...updateData, timeline: newTimeline } as Order;
  if (mapped.status === OrderStatus.SUCCESS) {
