...
export const PERMISSION_ERROR_CODES: readonly string[] = [
 ...
 */
export function isPermissionError(errCode?: string): boolean {
 ...
 * @param errCode - 错误码
 */
export function isNetworkError(errCode?: string): boolean {
 ...
 */
export function getFriendlyMessage(errCode?: string, fallbackMsg?: string): string {
  if (errCode) {
    // 精确命中映射表
