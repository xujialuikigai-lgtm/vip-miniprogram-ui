...
Page({
     ...
     */
    async loadDetail(keepForm = false) {
     ...
     */
    initSelection(packages, keepForm) {
        let target;
        // 1. 再买一次预选
        ...
        // 2. 默认套餐
        if (!target)
            target = packages.find((p) => p.isDefault);
        // 3. 首个套餐
        if (!target)
            target = packages[0];
    ...
    onMemberTypeTap(e) {
        ...
        if (!type || type === this.data.activeMemberType)
            return;
    ...
    refreshMemberType(memberType, preferPackageId) {
        ...
        if (!selected)
            selected = visiblePackages.find((p) => p.isDefault);
        if (!selected)
            selected = visiblePackages[0];
        this.setData({
    ...
    onPackageTap(e) {
        ...
        if (!packageId || packageId === this.data.selectedPackageId)
            return;
        const selected = this.data.visiblePackages.find((p) => p.packageId === packageId);
        if (!selected)
            return;
        this.setData({
     ...
     */
    accountModeOf(pkg) {
        const variants = (pkg && pkg.accountVariants) || [];
        const hasQQ = variants.some((v) => v.accountType === 'qq');
        const hasPhone = variants.some((v) => v.accountType === 'phone');
        if (hasQQ && hasPhone)
            return { phone: true, qq: true, placeholder: '请输入手机号或QQ号' };
        if (hasQQ && !hasPhone)
            return { phone: false, qq: true, placeholder: '请输入QQ号' };
        return { phone: true, qq: false, placeholder: '请输入绑定手机号' };
    },
    /** 按所选套餐支持的账号形式校验输入是否合法 */
    isAccountValid(value, pkg) {
        const v = String(value || '').trim();
        const mode = this.accountModeOf(pkg);
        const phoneOk = isValidPhone(v);
        const qqOk = /^[1-9]\d{4,9}$/.test(v);
        if (mode.phone && mode.qq)
            return phoneOk || qqOk;
        if (mode.qq && !mode.phone)
            return qqOk;
        return phoneOk;
    },
    /** 当前套餐的账号输入错误提示 */
    accountInputLabel(pkg) {
        const mode = this.accountModeOf(pkg);
        if (mode.phone && mode.qq)
            return '手机号或QQ号';
        if (mode.qq && !mode.phone)
    ...
    toDisplayPackage(pkg) {
        const pricePending = !pkg.online || pkg.price <= 0;
        return Object.assign(Object.assign({}, pkg), { pricePending, priceText: pricePending ? '待配置' : formatPrice(pkg.price), faceValueText: pkg.faceValue ? formatPrice(pkg.faceValue) : '' });
    },
    /**
    ...
    async onBuyTap() {
        ...
        const form = this.selectComponent('#dynamicForm');
        if (form) {
        ...
        if (fresh.success && fresh.data) {
            const latest = (fresh.data.packages || []).find((p) => p.packageId === pkg.packageId);
    ...
    async onConfirmPay() {
        ...
        try {
            // 1. 创建订单（锁定实付金额）
            const createRes = await requestSilent({
                name: 'order',
                action: 'create',
                data: {
            ...
            const payRes = await requestSilent({
                name: 'payment',
                action: 'unifiedOrder',
                data: { orderId }
            });
     ...
     */
    handleOrderError(errCode, errMsg) {
        this.setData({ confirmLoading: false });
