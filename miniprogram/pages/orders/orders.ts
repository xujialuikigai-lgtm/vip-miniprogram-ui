...
/** 页面状态机 */
type OrdersState = 'loading' | 'success' | 'empty' | 'error';

/** 再买一次校验返回（与云函数 order.rebuyCheck 返回结构对齐） */
interface RebuyCheckData {
  available: boolean;
...
Page({
  ...
  onLoad(this: any, options: { status?: string; tab?: string }) {
  ...
  onShow(this: any) {
    ...
    const pending = app && app.globalData && (app.globalData as any).orderTab;
    if (pending && VALID_TABS.indexOf(pending) !== -1) {
      (app.globalData as any).orderTab = '';
   ...
   */
  async onRefresh(this: any, e: WechatMiniprogram.CustomEvent) {
    const orderId = e.detail && e.detail.orderId;
    if (!orderId || this.data.refreshingId) return;

    this.setData({ refreshingId: orderId });

    const res = await requestSilent<{ order: Order }>({
      name: 'order',
      action: 'refresh',
      data: { orderId }
    });
    ...
    const list = this.data.list.map((item: Order) =>
      item.orderId === orderId ? { ...item, ...latest } : item
    );
   ...
   * 已下架 → 弹窗提示（Req 7.7）
   */
  async onRebuy(this: any, e: WechatMiniprogram.CustomEvent) {
    if (this._rebuyChecking) return;
    ...
    let data: RebuyCheckData;
    try {
      data = await request<RebuyCheckData>({
        name: 'order',
        action: 'rebuyCheck',
