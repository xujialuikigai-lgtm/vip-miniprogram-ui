...
/** 订单详情接口返回结构（与云函数 order.detail 对齐） */
 ...
 * @param status 订单状态
 */
function getEstimatedText(status: OrderStatus): string {
  switch (status) {
 ...
 * @param account 脱敏账号
 */
function buildHeroDesc(order: Order, account: string): string {
...
Page({
   ...
   * @param data 订单详情数据
   */
  applyDetail(this: any, data: DetailData) {
  ...
  async onRefresh(this: any) {
    ...
    const res = await requestSilent<DetailData>({
      name: 'order',
      action: 'refresh',
      data: { orderId: this._orderId }
    });
   ...
   */
  async onRebuy(this: any) {
    if (this._rebuyChecking) return;
    const order: Order | null = this.data.order;
    if (!order) return;

    this._rebuyChecking = true;
    const res = await requestSilent<{ available: boolean; reason?: string }>({
      name: 'order',
      action: 'rebuyCheck',
