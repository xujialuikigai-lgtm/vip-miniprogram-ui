...
Page({
    ...
    async loadDetail() {
        ...
        const packages = (res.data.packages || product.packages || []).map((p) => this.normalizePackage(p));
        // 保存原始套餐快照（深拷贝）用于改价检测
        this._originalPackages = packages.map((p) => (Object.assign({}, p)));
        // 售前规则：缺省时回填默认模板
    ...
    /** 为套餐附加展示用价格文案 */
    decoratePackage(p) {
        return Object.assign(Object.assign({}, p), { priceText: p.price.toFixed(2), costText: p.costPrice.toFixed(2) });
    },
    /* ============================================================
     * 基础信息表单事件
     * ============================================================ */
    ...
    onDeletePackage(e) {
        ...
        wx.showModal({
            ...
            success: (res) => {
                if (!res.confirm)
                    return;
                const packages = this.data.packages.slice();
    ...
    onSavePackage() {
        ...
        const existing = index >= 0 ? this.data.packages[index] : undefined;
        const pkg = {
        ...
        if (pkg.isDefault) {
            packages = packages.map((p, i) => (Object.assign(Object.assign({}, p), { isDefault: i === (index >= 0 ? index : packages.length - 1) })));
        }
        this.setData({
            packages: packages.map((p) => this.decoratePackage(p)),
            packagePopupVisible: false
    ...
    async onSave() {
        if (this.data.saving)
            return;
        const packages = this.data.packages.map((p) => this.stripPackage(p));
        // 1. 前端必填校验（Req 24.6）
        ...
        if (this.data.isEdit && this.isPriceChanged(packages)) {
            const ok = await this.confirmModal('改价提示', '新价格只影响后续订单；已支付订单仍按支付时锁定价格处理。是否继续保存？');
            if (!ok)
                return;
        }
    ...
    validateForm(packages) {
        ...
        if (!this.data.categoryId)
            return '请选择商品分类';
        if (!(Number(this.data.shunshiGoodsId) > 0))
            return '请填写接口商品ID';
    ...
    /** 检测套餐价格是否相对原始快照发生变更 */
    isPriceChanged(packages) {
        const originals = this._originalPackages || [];
    ...
    async doSave(packages) {
        this.setData({ saving: true });
        // 合并原始商品未编辑字段（attachTemplate / tags / brandIcon 等），覆盖已编辑字段
        const base = this._rawProduct || {};
        const product = Object.assign(Object.assign({}, base), { name: this.data.name.trim(), categoryId: this.data.categoryId, categoryName: this.data.categoryName, rechargeMethod: this.data.rechargeMethod, accountType: this.data.accountType, shunshiGoodsId: Number(this.data.shunshiGoodsId) || 0, autoActivate: this.data.autoActivate, online: this.data.online, sortWeight: Number(this.data.sortWeight) || 0, packages, rules: this.data.rules });
