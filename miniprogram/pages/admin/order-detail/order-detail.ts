...
/** 接口调用日志单项（与云函数 orderDetail 返回的 logs 对齐） */
...
/** 订单详情操作按钮状态（与云函数 orderDetail 返回的 actions 对齐） */
...
/** 管理端订单（在前端精简 Order 基础上补充管理端可见字段） */
...
interface AdminDetailData {
  order: AdminOrder;
  /** 后端已格式化的时间轴（status / statusName / time / desc） */
  timeline: Array<{ status: string; statusName: string; time: string; desc: string }>;
  logs: InterfaceLog[];
  actions: OrderActions;
}

/** retryActivation / 部分操作返回的轻量结果 */
 ...
 * @param order 订单数据
 */
function extractAccount(order: Order | null): string {
...
Page({
  data: {
    ...
    pageState: PageState.LOADING as PageState,
    PageState,
  ...
  onLoad(this: any, options: { orderId?: string }) {
  ...
  async loadDetail(this: any) {
    ...
    const res = await requestSilent<AdminDetailData>({
      name: 'admin',
      action: 'orderDetail',
      data: { orderId: this._orderId }
    });

    if (this.handlePermissionError(res)) return;
   ...
   * @param data 订单详情数据
   */
  applyDetail(this: any, data: AdminDetailData) {
   ...
   */
  handlePermissionError(this: any, res: { success: boolean; errCode?: string; errMsg?: string }): boolean {
   ...
   */
  async onRetryActivation(this: any) {
    if (this.data.retrying || !this.data.actions.canRetry) return;
    this.setData({ retrying: true });

    const res = await requestSilent<SimpleActionResult>({
      name: 'admin',
   ...
   * 查询接口状态（8.6）
   */
  async onQueryShunshi(this: any) {
    if (this.data.querying || !this.data.actions.canQuery) return;
    this.setData({ querying: true });

    const res = await requestSilent<{ shunshiStatus: number; status: OrderStatus }>({
      name: 'admin',
      action: 'queryShunshi',
      data: { orderId: this._orderId }
    });
   ...
   * 打开退款表单弹窗（6.1）
   */
  onShowRefund(this: any) {
    if (!this.data.actions.canRefund) return;
    this.setData({
   ...
   */
  onRefundFormVisibleChange(this: any, e: WechatMiniprogram.CustomEvent) {
   ...
   * 选择失败原因分类
   */
  onReasonChange(this: any, e: WechatMiniprogram.CustomEvent) {
   ...
   * 输入补充说明
   */
  onNoteInput(this: any, e: WechatMiniprogram.CustomEvent) {
   ...
   * 二次确认弹窗 - 确认发起退款（6.1）
   */
  async onConfirmRefund(this: any) {
   ...
   * 复制订单号 / 三方单号
   */
  onCopy(this: any, e: WechatMiniprogram.CustomEvent) {
